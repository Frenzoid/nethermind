...
public class WithdrawalContract : CallableContract, IWithdrawalContract
{
    ...
    public void ExecuteWithdrawals(BlockHeader blockHeader, IBlockTracer blockTracer, UInt256 failedMaxCount, IList<ulong> amounts, IList<Address> addresses)
    {
        ArgumentNullException.ThrowIfNull(blockHeader);
        ArgumentNullException.ThrowIfNull(amounts);
        ArgumentNullException.ThrowIfNull(addresses);

        Call(blockHeader, blockTracer, "executeSystemWithdrawals", Address.SystemUser, GasLimit, failedMaxCount, amounts, addresses);
    }
}
